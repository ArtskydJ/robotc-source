...
// !!!chassisAlignToLine is in BETA and needs to be tested!!!!
...
void testAutonomous()
{
	...
	// Going back to tile
	while (!chassisGoByGyro(30, gyro)) delay(15);
	...
	*/
	...
	liftSet(0);
	while (!chassisGoByTicks(575, 575)) {
		liftGoToHeight(bridge); delay(15);
	}
	while (!chassisGoByGyro(89, gyro)) delay(15);
	chassisResetIME();
	chassisSetTank(127, 127); delay(100);
	while (!lineSensorDetected(aLINE_CENT)) delay(15);
	delay(100);
	...
	while (!chassisGoByGyro(180, gyro)) {
		delay(15);
	}
	chassisResetIME();
	while (!chassisGoByTicks(1500, 1500)) {
		liftGoToHeight(rollover); delay(15);
	}
	while (!chassisGoByGyro(270, gyro)) {
		liftGoToHeight(ground); delay(15);
	}
	chassisResetIME();
	while (!chassisGoByInches(80, 80)) delay(15);
}