...
#pragma config(Motor,  port8,           plunger_servo, tmotorNormal, openLoop)
...
#define START_STEP (int)1 //you can start your script at something other than 1
...
int step = START_STEP ;// set START_STEP define at your desired script starting step
...
int plunger_cmd = HOLD;
int state_plunger = HOLD;
int last_state_plunger = HOLD;
int drop1_cmd = 0;
...
task main()
{
  ...
  while(true)
  {
    ...
    if(SensorValue(kill_sw) < 127) //using analog port since out of discretes
    {
      bIfiAutonomousMode = false;
    ...
    {
      autonomous_control(); //get commands from scripting functions
    }
    else
    {
      operator_control();  //else get commands from processed joystick inputs
    }
    ...
    plunger_function();//process plunger_cmd for griper servo
...
void lift_function()
{
  ...
  {  //use a single lift command and let the limit switches direct it to the proper motors
    ...
    { // switch control input to outer lift if(SensorValue(outer_upper_limit_sw == CLOSED) pwm_lift = 0 ;
      ...
      motor[rt_inner_lift] =  10 ;
      motor[lt_inner_lift] =  -10 ;
...
//plunger_function() controls the tube griper at the end of the plunger
//It computs a plunger_state and actuates the gripper servo
...
void plunger_function()
{
  ...
  //Plunger Servo
  if(plunger_cmd==HOLD && !bHold && bDrop_done) //RT_7Up
  {
    reset_plunger();
    bHold = true;
    //state_plunger = HOLD;
    motor[plunger_servo] = 127 ; //set hold servo cmd

  }
  else
    if(plunger_cmd==DROP  & !bDrop && bDrop_done)
  {
    reset_plunger();
    bDrop = true;
    //state_plunger = DROP;
    motor[plunger_servo] = -60 ; //set drop servo cmd

  }
  else
    if(plunger_cmd==DROP1 && !bDrop1 && bDrop_done)
  {
    reset_plunger();
    bDrop1 = true;
    //state_plunger = DROP1;
    drop1_cmd= (last_state_plunger == HOLD)*10 + (last_state_plunger == DROP)*70 ;
    motor[plunger_servo] = drop1_cmd ;

  }
  if(!bDrop_done && time1[T1]> 250)  //give some time for the tubes to drop
  {
    bDrop_done = true;
    state_plunger = plunger_cmd;
  }

  last_state_plunger = state_plunger;
}
...
{  /*
  ...
  */
  ...
  else
  {
    ...
    if(!bstart_track && (blt_line|| bct_line|| brt_line) ) {bstart_track=true;}
    if(bstart_track)
    {
...
//Outputs: pwm_sym,pwm_turn,pwm_lift,plunger_cmd,base_align_cmd and bauto_line_enable
...
void operator_control()
{
  ...
  //Generate plunger and base_align commands
  if(RT_7Up) plunger_cmd = HOLD;
  else
    if(RT_7Down) plunger_cmd = DROP;
  else
    if(RT_7Right) plunger_cmd = DROP1;
...
void autonomous_control()
{
  ...
  else  //check for auto_debug discretes
    if(bstarted_script)
  {
...
//process.  First a single tube is dropped and then the remaining tubes are lowered to the end of the plunger
...
int set_grip(int goal, int ms_wait)
{
  ...
  if(sys_reset)
  {
    if(goal==HOLD || goal==DROP || goal==DROP1) plunger_cmd=goal;
  ...
  if( (state_plunger != goal_temp ) && !imdone ) return 0;
  else
...
//Arm autonomous mode (bauto_debug_mode=true) : Push 5Up and 6Up simultaneously for 3 seconds and release
...
void auto_debug_function()
{
  ...
  if (RT_5Up && RT_6Up && !b5sec_in_auto_debug_mode )//button 5u and 6u pushed for 3 sec ->auto_debug_mode
  {
    if(binit_auto_timer)
    {time100[T2] = 0;
      pwm_sym = 0;
  ...
  if(RT_5Up && (bstarted_script || b5sec_in_auto_debug_mode))
  {
    bauto_debug_mode = false;
...
void autonomous_script() {
    ...
    if(step  == 3)
  {
    ...
    if(step  == 4)
  {
    done = set_grip(DROP, 200); //open plunger 200 ms delay

  }
  else
    if(step  == 5)
  {
    done = set_lift(LIFT_DOWN,-40); // lower plunger into tubes at pwm = -40
    ...
    if(step  == 15)
  {
    ...
    if(move_encoder(-240,90)==1 || bback_bumper) done = 1 ; //move back till bumper hit, 90 pwm
